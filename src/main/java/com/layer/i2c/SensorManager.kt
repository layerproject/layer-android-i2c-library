...
/**
 * Manages low-level I2C communication with AS7343 spectral sensors.
 * Provides methods to read/write registers, configure the sensor,
 * and perform measurements. This class interacts directly with I2cNative.
 */
class SensorManager {
    companion object {
        private const val TAG = "I2C-AS7343-Manager"
    ...
    fun readAllChannels(fd: Int): Map<String, Int> {
     ...
     /**
     * Helper to extract the primary 14 channels into a map with simplified names.
     * @param fullData The map returned by readAllChannels.
     * @return Map containing primary channels with names like "F1", "NIR", "VIS", etc.
     */
    fun extractPrimaryChannels(fullData: Map<String, Int>): Map<String, Int> {
        ...
        primaryChannelKeys.forEachIndexed { index, key ->
            ...
            primaryMap[simpleName] = fullData[key] ?: 0 // Get value using the register-based key
        }
        return primaryMap
    }
    ...
    fun setIntegrationTime(fd: Int, atime: Int, astep: Int) {
        if (fd < 0) return
